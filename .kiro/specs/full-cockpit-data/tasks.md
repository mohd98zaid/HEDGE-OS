...
- [ ] C.5 hedge-exec fill capture — subscribe to broker fill streams (Upstox WS, Angel One postback URL); publish `exec.fill.<SYM>` and `exec.trade.closed` — files: `crates/hedge-exec/src/fills.rs` — references: REQ-7.3, REQ-7.6, REQ-7.7
