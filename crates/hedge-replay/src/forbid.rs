//! Architectural-prohibition manifest for `hedge-replay`.
//!
//! Mirrors `crates/hedge-warmcache/src/forbid.rs` and
//! `crates/hedge-bus/src/forbid.rs`. The list is the source of truth
//! for the CI gate from task 8.1 — `scripts/check-forbidden-deps.sh`
//! and `scripts/check-forbidden-source.sh` walk every Hot_Path-adjacent
//! crate's transitive dependency graph and source tree against the
//! same set of prohibitions (R30.1–R30.8).
//!
//! The Replay_Engine is doubly subject to these rules:
//!
//! * It binds the Execution_Engine to [`hedge_broker_simulated::SimulatedBroker`]
//!   when `ReplayMode::On` (R22.4) — i.e. it sits on the order-decision path
//!   for replays. Replay determinism (Property 12) requires the same
//!   purity as live trading.
//! * It sinks every Hot_Path event to disk and Redis Streams. Pulling a
//!   Python runtime or cloud SDK into the recorder would link those
//!   crates into every Hot_Path image transitively.

/// Crate names whose transitive presence on `hedge-replay` is forbidden.
///
/// Categories (verbatim mirror of `hedge-bus` / `hedge-warmcache`):
///
