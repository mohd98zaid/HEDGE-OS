"""Bus abstractions for the AI_Trade_Journal_Engine (task 27.1).

The engine talks to NATS through three surfaces, mirroring the layout
:mod:`hedge_warm_ai.prev_day.bus` already established for the
Previous_Day_Memory_Engine:

* :class:`JournalBusPublisher` — fire-and-forget publishes onto
  ``ai.journal.entry``.
* :class:`JournalBusSubscriber` — subscribe to ``exec.trade.closed``
  and dispatch the decoded payload to a coroutine.
* :class:`JournalRequestReplyServer` — register a coroutine handler
  for ``mem.journal.query`` request-reply.

The actual NATS client (``nats-py``) is wired only at the service-entry
boundary; tests pass :class:`InMemoryJournalBus` so the engine can be
exercised without the broker. This mirrors the
:class:`hedge_warm_ai.prev_day.bus.InMemoryPrevDayBus` pattern used
elsewhere in the Warm_AI_Pipeline.

The publish / subscribe / request callables match the same
``(subject, payload_bytes)`` shape as
:class:`hedge_warm_ai.ollama_client.NatsDegradedPublisher`, so a service
can plug an existing ``async def publish(subject, payload)`` helper
directly into :class:`JournalBusPublisher`.
"""
...
JournalPublishCallable = Callable[[str, bytes], Awaitable[None]]

#: Subscription-side handler: ``async def handle(payload: bytes) -> None``.
JournalSubscribeCallable = Callable[[bytes], Awaitable[None]]

#: Request-reply server-side handler: ``async def handle(payload: bytes) -> bytes``.
JournalReplyHandler = Callable[[bytes], Awaitable[bytes]]
...
class JournalBusPublisher(Protocol):
    """Sink for ``ai.journal.entry`` event publications."""

    async def publish(self, subject: str, payload: bytes) -> None: ...


class JournalBusSubscriber(Protocol):
    ...
    callback never blocks the bus reader. The engine relies on this
    invariant — it dispatches the slow Ollama narrative work via
    :func:`asyncio.create_task` so even a fast bus can return
    immediately from the trade-closed callback.
    """

    def subscribe(
        self, subject: str, handler: JournalSubscribeCallable
    ) -> "AsyncContextManager[None]": ...


class JournalRequestReplyServer(Protocol):
    ...
    def serve(
        self, subject: str, handler: JournalReplyHandler
    ) -> "AsyncContextManager[None]": ...
...
class _Subscription:
    subject: str
    handler: JournalSubscribeCallable


@dataclass
class _ReplyServer:
    subject: str
    handler: JournalReplyHandler


class InMemoryJournalBus:
    """Single-process in-memory bus implementing all three protocols.

    Designed for unit tests and the local service integration fixture.
    Behaviour is intentionally simple:
    ...
    * ``request(subject, payload, timeout_s)`` invokes the registered
      :class:`JournalReplyHandler` for ``subject`` and returns its
    ...
    * Captured publications are kept in :attr:`captured` for assertion
    ...
    async def request(
        ...
        Used by tests to drive ``mem.journal.query`` without spinning
...
class CallableJournalPublisher:
    """Concrete :class:`JournalBusPublisher` wrapping a publish callable.
    ...
    publish_callable: JournalPublishCallable
